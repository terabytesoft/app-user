...
class UserModel extends ActiveRecord implements IdentityInterface
{
    ...
    public function attemptEmailChange($code)
    {
        ...
        if (empty($this->unconfirmed_email) || $token === null || $token->isExpired) {
            ...
            if (empty($this->unconfirmed_email)) {
                Yii::getApp()->session->setFlash('danger', Yii::t('user', 'An error occurred processing your request'));
                ...
                if ($this->module->emailChangeStrategy == Module::STRATEGY_DEFAULT
                    ...
                    Yii::getApp()->session->setFlash('success', Yii::t('user', 'Your email address has been changed'));
                }
                $this->save(false);
            }
        }
    ...
    public function block()
    {
        return (bool)$this->updateAttributes([
            'blocked_at' => time(),
            'auth_key'   => Yii::getApp()->security->generateRandomString(),
        ]);
    }
    ...
    {
        if ($insert) {
            $this->setAttribute('auth_key', Yii::getApp()->security->generateRandomString());
            if (Yii::getApp() instanceof WebApplication) {
                $this->setAttribute('registration_ip', Yii::getApp()->request->userIP);
            }
        }
